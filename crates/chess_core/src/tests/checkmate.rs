...
#[test]
fn detects_basic_checkmate_position() {
    ...
    set_piece(&mut state, "g7", PieceColor::White, PieceKind::Queen);
    set_piece(&mut state, "f6", PieceColor::White, PieceKind::King);

    assert!(matches!(
        status(&state),
        GameStatus::Checkmate(PieceColor::White)
    ));
}
