# Chess Platform Rules Doc

## Objective
Define mandatory operating rules for planning and implementation so each prompt is auditable, progress-aware, and traceable to explicit decisions.

## Working Rules And Guardrails
- `design.md` and `rules.md` are mandatory control artifacts for every prompt.
- Every prompt requires two log entries: one before execution and one after execution.
- Progress must always be tracked as both percentage and remaining checklist.
- `progress_remaining_percent` must always equal `100 - progress_after_percent`.
- Plan or process changes must be recorded in the same turn they are introduced.
- Every after-entry must include files touched and decisions changed.
...
## Mandatory Before/After Prompt Procedure
1. Read `/Users/aaditnilay/Documents/chess_repo/docs/design.md`.
2. Read `/Users/aaditnilay/Documents/chess_repo/docs/rules.md`.
3. Append a `PromptTurnRecord` before-entry with prompt summary, plan, approach, progress before, and remaining tasks.
4. Execute the requested planning/implementation work.
5. Append a `PromptTurnRecord` after-entry with completion summary, progress after, remaining tasks, blockers/risks, files touched, and decisions changed.
6. Update milestone status and weighted totals in `/Users/aaditnilay/Documents/chess_repo/docs/design.md`.
7. If process changed, update relevant rules in `/Users/aaditnilay/Documents/chess_repo/docs/rules.md`.

## Progress Calculation Rule
- Formula:
  - `overall_progress_percent = sum(completed milestone weights and partials)`
  - `progress_remaining_percent = 100 - overall_progress_percent`
- Constraint:
  - Milestone weights must total exactly `100`.

## Change-Control Rule
- Any change to scope, architecture, milestones, or execution process must be logged in the turn where it occurred.
- Changes are valid only if both docs are updated when applicable:
  - Design-impacting changes -> update `design.md`.
  - Process-impacting changes -> update `rules.md`.

## Logging Format And Minimum Required Fields
- Each log entry must include all `PromptTurnRecord` fields.
- `turn_id` format: `T-####-BEFORE` or `T-####-AFTER`.
- `timestamp_local` format: local ISO-like timestamp (e.g., `2026-02-13T23:37:49-0500`).
- `files_touched` must list absolute file paths.
- `decisions_added_or_changed` must capture any new default, policy, or architecture decision.

## Compliance Checklist Per Turn
- [ ] Read `design.md` and `rules.md` before work starts.
- [ ] Logged one before-entry.
- [ ] Logged one after-entry.
- [ ] Updated progress percent and remaining checklist.
- [ ] Verified `progress_remaining_percent = 100 - progress_after_percent`.
- [ ] Updated milestones in `design.md` if progress changed.
- [ ] Updated `rules.md` if process changed.
- [ ] Recorded files touched and decisions changed.

## Execution Rules Registry

| rule_id | description | required | verification_method |
| --- | --- | --- | --- |
| R1 | Maintain `design.md` and `rules.md` as mandatory control artifacts for every prompt. | yes | Check both files exist and include current turn logs. |
| R2 | Create a before-entry for every prompt before execution starts. | yes | Validate one `T-####-BEFORE` record per handled prompt. |
| R3 | Create an after-entry for every prompt after execution ends. | yes | Validate one `T-####-AFTER` record per handled prompt. |
| R4 | Track progress using both percentage and remaining checklist. | yes | Confirm both values are present in each after-entry. |
| R5 | Enforce progress integrity equation. | yes | Confirm `progress_remaining_percent = 100 - progress_after_percent`. |
| R6 | Update milestones in `design.md` when progress moves. | yes | Compare milestone status before/after each completed turn. |
| R7 | Record files touched and decisions changed in every after-entry. | yes | Ensure both fields are non-empty for each after-entry. |
| R8 | Bootstrap exception: if docs do not yet exist, create them and backfill the before-entry in the same turn. | yes | Confirm first turn includes explicit bootstrap before/after records. |
